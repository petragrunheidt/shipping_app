  ...
  describe '#valid?' do
    ...
    context 'comparison' do
      it 'false when max is less than min' do
        # Arrange
        wt = FactoryBot.build(:weight_table, min: 10, max: 5)
      ...
      it 'false when min is equal to zero' do
        # Arrange
        dl = FactoryBot.build(:weight_table, min: 0, max: 50)
      ...
      it 'false when min is less than zero' do
        # Arrange
        dl = FactoryBot.build(:weight_table, min: -1, max: 50)
      ...
      it 'false when min is equal than last table input max' do
        tm = FactoryBot.create(:transport_mode, min_weight: 10, max_weight: 100)
        dl1 = FactoryBot.create(:weight_table, min: 10, max: 30, transport_mode: tm)
        dl2 = FactoryBot.build(:weight_table, min: 30, max: 50, transport_mode: tm)
