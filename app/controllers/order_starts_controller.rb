class OrderStartsController < ApplicationController
  ...
  def new
    ...
    @transport_modes = TransportMode.where(status: :active).where('max_weight > ?', @order.weight).where(
      'min_weight < ?', @order.weight
    )
  end
  ...
  def create
    ...
    @order_start.vehicle = @order_start.transport_mode.vehicles.where(status: :available).find_by('maximum_load > ?',
                                                                                                  @order.weight)
  ...
  def destroy
    @order.pending!
    return unless @order.order_start.destroy

    redirect_to @order, notice: 'Inicie novamente a Ordem de Serviço.'
  end
